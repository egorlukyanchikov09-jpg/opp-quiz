(() => {
  ...
  function finishQuiz(fromCheat) {
    ...
    $("result-details").innerHTML = state.cheated
      ...
      : `Студент: <strong>${state.name}</strong> · ${state.group}<br>
         Модуль: ${state.selectedModule}<br>
  ...
  function buildPersonalExcel() {
    ...
    const summary = [
      ["Тестирование ОПП · Алабуга"],
      ["Дата", state.endTime.toLocaleString("ru-RU")],
      ["ФИО", state.name],
      ["Направление и группа", state.group],
  ...
  function buildMasterExcel() {
    ...
    history.forEach((h) => {
      rows.push([h.date, h.name, h.group, h.modules, h.status, h.correct, h.total, h.pct, h.xp, h.duration]);
    });
  ...
  function downloadMaster() {
    XLSX.writeFile(buildMasterExcel(), `ОПП_все_результаты_${Date.now()}.xlsx`);
  }
  ...
  function openAdminPanel() {
    ...
    tbody.innerHTML = "";
    if (!history.length) {
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:#8AA0B5;padding:1.5rem;">Пока нет результатов</td></tr>`;
      return;
    }
    history.slice(0, 100).forEach((h) => {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td>${h.date}</td><td>${h.name}</td><td>${h.group}</td>
        <td>${h.modules}</td><td>${h.pct}%</td><td>${h.xp}</td>
  ...
  function launchConfetti() {
    ...
    for (let i = 0; i < 140; i++) {
      pieces.push({
        ...
        speed: 2 + Math.random() * 4, rot: Math.random() * 360, rotSpeed: -5 + Math.random() * 10
      });
    }
    let frames = 0;
    function frame() {
