(() => {
  ...
  function playSiren() {
    try {
      ...
      const makeTone = (freq, type, delay) => {
        const osc = ctx.createOscillator();
        const g = ctx.createGain();
        osc.type = type; osc.frequency.value = freq; g.gain.value = 0.5;
        osc.connect(g); g.connect(master);
        osc.start(ctx.currentTime + delay);
        state.sirenNodes.push(osc, g);
        return osc;
      };
      const o1 = makeTone(780, "sawtooth", 0);
      const o2 = makeTone(980, "sawtooth", 0);
      ...
      lfo.start(); state.sirenNodes.push(lfo, lfoGain);
      const pulse = ctx.createOscillator();
      const pulseG = ctx.createGain();
      pulse.frequency.value = 4; pulseG.gain.value = 0.3;
      pulse.connect(pulseG); pulseG.connect(master.gain);
      pulse.start(); state.sirenNodes.push(pulse, pulseG);
      setTimeout(stopSiren, 8000);
    } catch (e) { console.warn("Siren failed", e); }
  }
  function stopSiren() {
    try {
      state.sirenNodes.forEach((n) => { try { n.stop?.(); } catch (_) {} try { n.disconnect?.(); } catch (_) {} });
      state.sirenNodes = [];
      if (state.sirenCtx) { state.sirenCtx.close?.(); state.sirenCtx = null; }
    } catch (_) {}
  }
  function markCheat() {
    ...
    playSiren();
    setTimeout(() => finishQuiz(true), 3500);
  }
  ...
  function startQuiz() {
    ...
    if (isAdminName(name)) { state.name = name; state.isAdmin = true; openAdminPanel(); return; }
  ...
  function launchConfetti() {
    ...
    const pieces = []; const colors = ["#0055A5", "#4DA3FF", "#003D7A", "#00C853", "#FF9100"];
  ...
  function runLoading() {
    const bar = $("load-bar"); const pct = $("load-pct"); let p = 0;
